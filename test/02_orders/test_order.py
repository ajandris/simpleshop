...
def test_create_order(base_cart_with_discount):
    ...
    assert cart_totals["cart_hash"] == get_order_hash(
        order
    ), "The cart and the order hashes are different"


def test_re_build_order(
    base_order_with_discount, base_cart_with_discount_1000_min_subtotal
):
    order_status = OrderStatusFactory(code="PENDING")
