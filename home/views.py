...
def index(request):
    ...
    categories = list(
        Category.objects.filter(is_featured=True).order_by("title")
    )
    products = Product.objects.filter(category__in=categories).order_by(
        "category__title", "-inserted_at"
    )
...
def profile_generalinfo(request):
    ...
    user = (
        a_user.objects.filter(username=request.user.username).values().first()
    )
    email = (
        EmailAddress.objects.filter(user_id=request.user.id).values().first()
    )
...
def profile_addresses_edit_show(request):
    ...
    address = get_object_or_404(
        Address,
        id=int(request.POST.get("address_id", 0)),
        profile=user_profile,
    )
...
def writeme(request):
    """
    View function for the contact/writeme page.
    """
    ctx = {
    ...
    if request.method == "POST":
        ...
        Hi, {name}! 
        We have received your message sent from The Olde Chrismas Market
        contact form.
        You wrote us:
        {message}
        """
